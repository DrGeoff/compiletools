...
class Namer:
    ...
    def topbindir(self):
        ...
        """
        bindir = self._bindir
        if not os.path.isabs(bindir) and os.sep in bindir:
            return bindir.split(os.sep)[0] + os.sep
        return bindir
    ...
    def object_dir(self, sourcefilename=None, file_hash=None):
        ...
        * Concurrent writers — peer ``make`` jobs on one box, or many
        ...
        if file_hash is not None:
            return os.path.join(self._cas_objdir, file_hash[:2])
        return self._cas_objdir
    ...
    def executable_dir(self, sourcefilename=None):
        ...
        if sourcefilename:
            ...
            if subdir:
                return os.path.join(self._bindir, subdir)
        return self._bindir
