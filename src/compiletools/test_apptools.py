...
class TestFuncs:
    ...
    def test_set_project_version_escaping(self):
        ...
        for version, description in test_cases:
            ...
            version_flag = next(p for p in parts if p.startswith("-DCAKE_PROJECT_VERSION="))
    ...
    def test_set_project_version_integration(self):
        ...
        for version in test_versions:
            ...
            try:
                ...
                run_result = subprocess.run([out_file], capture_output=True, text=True)

                print(f"  Program output: {run_result.stdout.strip()}")

                assert run_result.returncode == 0, f"Program failed with version '{version}': {run_result.stderr}"

                assert version in run_result.stdout, f"Version '{version}' not found in output: {run_result.stdout}"

                print("  ✓ Program executed successfully with correct version")
...
class TestConfig:
    ...
    def _test_variable_handling_method(self, variable_handling_method):
        ...
        """
        with uth.TempDirContext(), uth.EnvironmentContext({"CXXFLAGS": "-fdiagnostics-color=always -DVARFROMENV"}):
