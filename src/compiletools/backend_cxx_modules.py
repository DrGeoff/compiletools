...
def _pcm_command_hash(
    ...
    *,
    ...
    function does NOT re-filter that parameter; only the per-file
    ``magic_cpp_flags`` / ``magic_cxx_flags`` (which arrive un-filtered
    from the magic-flag pipeline) are filtered here. Symmetric with
    ``_pch_command_hash``.
    """
