"""Git blob-SHA hashing helpers for the working tree.

These functions are the library core behind the ``ct-git-sha-report`` CLI
(:mod:`compiletools.git_sha_report`) and are also consumed directly by
``global_hash_registry``. They live here -- separate from the CLI entry point --
so core library code can depend on the hashing helpers without importing a
command-line tool (keeping the entry point a leaf in the import graph).
"""
