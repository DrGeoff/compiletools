...
int main() {
    ...
    // Side wall reflects vx.
    {
        ...
        for (auto&& b : a.bricks.grid) b = false;  // std::vector<bool> yields a proxy, not bool&
    ...
    // Paddle reflects vy.
    {
        Arena a = initial(20, 12);
        for (auto&& b : a.bricks.grid) b = false;  // std::vector<bool> yields a proxy, not bool&
        a.bricks.grid[0] = true;  // keep one brick at screen (0,1) so classify() isn't Won;
    ...
    // Missing the ball (it goes past the paddle row) is a Loss.
    {
        Arena a = initial(20, 12);
        for (auto&& b : a.bricks.grid) b = false;  // std::vector<bool> yields a proxy, not bool&
        a.bricks.grid[0] = true;  // keep one brick so the only way to end is the miss
    ...
    // Clearing the last brick is a Win.
    {
        Arena a = initial(20, 12);
        for (auto&& b : a.bricks.grid) b = false;  // std::vector<bool> yields a proxy, not bool&
        a.bricks.grid[0] = true;        // single brick at (row 0, col 0), screen (0,1)
