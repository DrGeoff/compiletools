...
def _make_argparser(*, with_magic=False, with_hunter=False):
    """Build a configargparse parser pre-configured for header-deps tests."""
...
def test_macro_state_dependency_is_fixed():
    ...
    # Without DEBUG should include release.h; with DEBUG should include debug.h.
    # Before the fix, both cases would have included release.h due to cached results.
    assert has_release and not has_debug, (
        f"Test 1 should include release.h only, got release={has_release}, debug={has_debug}"
    )
    assert has_debug2 and not has_release2, (
        f"Test 2 should include debug.h only, got release={has_release2}, debug={has_debug2}"
    )
...
def test_hunter_respects_macro_state_changes():
    ...
    def create_hunter_with_macros(debug_enabled=False):
        ...
        cap = _make_argparser(with_magic=True, with_hunter=True)
