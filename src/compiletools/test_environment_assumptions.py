...
""")
    ...
    result = subprocess.run(["clang++", "-std=c++17", "-o", test_exe, test_cpp], capture_output=True, text=True)
...
def test_compiler_diagnostics_support(temp_dir):
    ...
    result = subprocess.run(
        [functional_compiler, "-fdiagnostics-color=auto", "-c", test_cpp, "-o", test_o], capture_output=True, text=True
    )
    ...
    if clangxx_path is not None:
        os.remove(test_o)
        result = subprocess.run(
            ["clang++", "-fdiagnostics-color=auto", "-c", test_cpp, "-o", test_o], capture_output=True, text=True
        )
...
""")
    ...
    result = subprocess.run(
        [functional_compiler] + optimization_flags + [test_cpp, "-o", test_exe], capture_output=True, text=True
    )
...
""")
    ...
    result = subprocess.run([functional_compiler, "-Wall", test_cpp, "-o", test_exe], capture_output=True, text=True)
