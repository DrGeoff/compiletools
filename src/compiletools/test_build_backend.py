...
class TestRunTests:
    ...
    def test_run_tests_no_timing_when_disabled(self, mock_realpath, mock_run, tmp_path):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
        backend = self._make_backend(tmp_path)
        # _make_backend may incidentally invoke ``subprocess.run`` (e.g.
        # ``git rev-parse`` for git_root resolution on a cold cache, which
        # can happen when this is the first test in an xdist worker). Only
        # invocations during ``_run_tests`` are relevant.
        mock_run.reset_mock()
