...
def clear_cache():
    ...
    :mod:`compiletools.apptools_compiler`; we fan out to its
    ``clear_cache`` so the exact same set of compiler caches is cleared as
    before the facade split (``_get_functional_cxx_compiler_cached``,
    ``compiler_identity``, ``compiler_kind``, ``compiler_default_cxx_std``,
    ``find_system_std_module_source``). The non-moved ``cached_pkg_config``
    is still cleared directly here. Net effect is identical to the previous
