...
def ensure_ct_lock_helper_py_in_path():
    """Ensure ct-lock-helper-py is available in PATH for all tests."""
    import shutil as sh
    if not sh.which('ct-lock-helper-py'):
        # Try to find it in common locations
        test_dir = os.path.dirname(__file__)
        repo_root = os.path.dirname(os.path.dirname(os.path.dirname(test_dir)))
        helper_path = os.path.join(repo_root, 'ct-lock-helper-py')

        if os.path.exists(helper_path):
            # Add repo root to PATH for tests
            os.environ['PATH'] = repo_root + os.pathsep + os.environ.get('PATH', '')
            print(f"\nAdded {repo_root} to PATH for ct-lock-helper-py", file=sys.stderr)
        else:
            pytest.skip("ct-lock-helper-py not found in PATH or repository root")
