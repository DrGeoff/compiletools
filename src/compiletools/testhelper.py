...
def _gitignored_under(src: Path) -> set[Path]:
    ...
    *directory*, failing every backend cell with FileExistsError.
    ...
    2.50.1 reports the ignored subdirectories). File-level output avoids
    that version-dependent directory fold. The fold done here is slightly
    wider than git's: a directory that is not itself ignored but whose
    every entry is gets pruned too.
