...
### Workspace-Relative Compile Paths (Round 3)

`apptools._inject_ffile_prefix_map` (tail of `_commonsubstitutions`) appends `-ffile-prefix-map=<gitroot>=<target>` to `CXXFLAGS`/`CFLAGS` (target via `--ffile-prefix-map-target`, default `.`) so DWARF / `__FILE__` / `.d` paths become workspace-independent and CAS artefacts are byte-identical across users; skipped per-slot when the user already supplied any `-f{file,debug,macro,canon}-prefix-map=`. Linker symmetry via sister functions `canonicalize_for_command` / `canonicalize_path_for_command` (target substituted, not `<GITROOT>`); `build_backend._create_link_rule` / `_create_shared_library_rule` route LDFLAGS through them so RPATHs and `--version-script` paths under the gitroot are target-prefixed in the argv. `canonicalize_for_cache_key` also rewrites the OLD side of those flag families, otherwise two users' auto-injected tokens would re-leak the gitroot through the very flag meant to scrub it.

**PCH/BMI byte-identity is not achievable across workspaces, but cross-user cache sharing still works.** gcc's PCH (`.gch`) and BMI (`.pcm` / `.gcm`) on-disk layouts encode pointer offsets that depend on the build cwd path *length* (empirically: same-length cwds yield identical bytes; differing lengths shift internal offsets). No combination of `-ffile-prefix-map`, `-fdebug-compilation-dir`, `PWD=/proc/self/cwd`, or symlink-chdir tricks fixes it; this is internal gcc serialisation that no compile-time flag controls. **What actually matters for cross-user CAS sharing is downstream `.o` byte-identity** — which IS achieved, via two pieces in the precompile rule emitters (`_create_pch_rules` for PCH, `_create_clang_module_interface_rules` for clang PCM): (1) the source path is passed *relative to* `self._anchor_root` (gated on `_is_under(source, anchor_root)`; outside-anchor sources stay absolute), and (2) `BuildRule.cwd = self._anchor_root` so the recipe runs from the gitroot. Backends honour `cwd` by prepending `cd <cwd> && ` inside the lock wrapper (make/ninja via `_wrap_compile_cmd(..., cwd=)`) or by passing `cwd=` to `subprocess.run` (shake/slurm via `execute_compile_rule(..., cwd=)` → `atomic_compile(..., cwd=)` → `_run_with_signal_forwarding(..., cwd=)`). Without this discipline, alice's PCH bakes `/home/alice/proj/...` into its DWARF `.debug_line_str` include-dir table and any consumer (including a peer reading from a shared cas-pchdir) inherits alice's paths into their `.o`.

For gcc C++20 modules, an additional leak: the per-build module-mapper file lives at `<bindir>/.module-mapper.txt` (outside the gitroot, so `-ffile-prefix-map` doesn't reach it), and its absolute path goes into `DW_AT_producer`'s recorded compile-flag string. `_compiler_module_flags_for` therefore appends `-gno-record-gcc-switches` whenever `-fmodule-mapper=` is present, suppressing flag-string recording for module-using compiles only — DW_AT_producer keeps the gcc version + target triple but drops the per-user mapper path. Acceptance gate `test_ffile_prefix_map.py`: the load-bearing tests are `test_two_checkouts_produce_byte_identical_o_with_shared_cas_p{ch,cm}dir` (alice and bob share a single cas-pchdir/cas-pcmdir and assert their `.o` outputs match — what users actually need from NFS-shared CAS). The companion `test_two_checkouts_produce_byte_identical_cas_p{ch,cm}dir` tests are kept `xfail(strict=False)` as **negative controls** that pin the empirical `.gch` / `.pcm` divergence — if a future change accidentally achieves byte-identity the marker can be dropped.
...
**Consumer-side wiring is `-include <cas-pchdir>/<hash>/<basename>`, NOT `-I <cas-pchdir>/<hash>`.** GCC's `#include "header"` resolution searches the source-file directory before any `-I` directory, so an `-I <cache>` flag is silently bypassed whenever the PCH header coexists with the consumer source — the realistic case for a private per-TU PCH (header and consumer `.cpp` co-located in the same directory). Source-dir resolution finds the bare `.h`, GCC then looks for a sibling `.gch` only beside that resolved path, never reaching the cache. The absolute `-include` form opens `<cache>/<basename>.gch` unconditionally (PCH lookup is sibling-to-resolved-path), and the consumer's own `#include "header"` is a no-op via the include guard. Anyone tempted to "simplify" the emitted flag back to `-I` will silently revert this fix; regression guard is `test_pch_bypass_bug.py` parameterized over make / ninja / cmake / bazel (asserts the consumer compile's `-H` output contains a `! …gch` PCH-used marker — `x …gch` reject markers also count as evidence the cache was reached, distinguishing the bypass bug from a separate flag-mismatch concern).

**Companion staging.** `_create_compile_rule` calls `_stage_pch_header_alongside_gch` to hardlink (with EXDEV-tolerant copy fallback) the source `.h` next to the `.gch` at `<cas-pchdir>/<hash>/<basename>`. Without it, gcc's `-include` directive errors `No such file or directory` whenever the cache is invalidated at consume time and gcc tries to fall back to the bare header. Hardlinks share inodes with the cas-pchdir master so the staging costs zero disk; the missing-source guard makes the function a silent no-op for mock-based unit tests with synthetic paths.

**Bazel-specific staging + dialect alignment.** Bazel's hermetic `CcCompileAction` rejects absolute paths outside the toolchain's `cxx_builtin_include_directories`, and its `additional_compiler_inputs` mechanism only accepts workspace-relative labels — so the cas-pchdir absolute path that `_create_compile_rule` emits cannot reach a bazel sandbox directly. `BazelBackend._bazel_pch_inputs_and_copts` (planning) records the cas → workspace mapping and rewrites the `-include` flag to `<workspace>/.ct-bazel-pch/<hash>/<basename>`; `_materialise_pch_stagings` (called from `_execute_build` AFTER `_prebuild_aux_artefacts` builds the .gch) hardlinks both the `.h` and the sibling `.gch` into `.ct-bazel-pch/`. Bazel symlinks each declared input into the action's exec root at the same workspace-relative path; gcc's PCH lookup finds the sibling `.gch` and validates. For PCH validation to actually succeed, the dialect bazel uses for the consumer compile must match the dialect ct-cake used for the PCH — bazel's `local_config_cc` autoconfig defaults to `-std=c++17` (strict ISO), which undefines built-ins like `unix`, `linux`, `__unix__` that ct-cake's gcc-default `-std=gnu++NN` mode kept defined, producing `PCH not used because 'unix' not defined` rejections. `_build_bazelrc_content` therefore unconditionally emits `build --cxxopt=-std=<X>` where `<X>` is the user's explicit `-std=` if set in CXXFLAGS, otherwise `apptools.compiler_default_cxx_std(args.CXX)` — a per-compiler probe (`<cxx> -dM -E -x c++ /dev/null`, parses `__cplusplus`, maps to `gnu++NN` — gcc-16 → gnu++20, clang-21 → gnu++17) that aligns bazel with whatever the compiler natively defaults to. The `gnu++` mode is the explicit choice: both gcc and clang default to gnu mode, and switching to strict `c++` would itself invalidate any prebuilt PCH that recorded gnu-mode built-ins. Regression guards are `test_pch_bypass_bug.py::test_consumer_compile_actually_opens_cached_pch[bazel]` (proves the cache is reached and used end-to-end) and `test_apptools.py::TestCompilerDefaultCxxStd` (pins the per-compiler dialect mapping).
...
All tools support `--variant=<config>` for build configuration selection and `--backend=<name>` for build system backend selection (make, ninja, cmake, bazel, shake).
