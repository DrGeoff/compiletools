...
Must hold in `wrap_compile_with_lock`/`wrap_link_with_lock` (Make/Ninja native flock) and `atomic_compile`/`atomic_link` (Shake via `trace_backend.py`). See `src/compiletools/CLAUDE.md` for the full rationale.
...
- `trace_backend.py` — the Shake backend (self-executing verifying traces).
...
- `test_no_slop_tests.py` is an `ast`-based lint over every `test_*.py`, and a **prek hook** (`no-slop-tests`), so it gates commits. Six build-failing rules: R1 no verification / R2 tautology / R3 bare `except:` / R4 mock-assert-only / R5 discarded `readouterr()` capture / R6 name promises creates·writes·removes·equals without the matching check. Each has a `frozenset` allowlist keyed `"<file>::<test>"`; every entry needs an inline justification and is typo-guarded by `test_allowlist_entries_are_live`. R4 is the one that routinely fires on *sound* tests — for dispatch wiring or a "work was skipped" guard a mock assertion is the only observable, so expect new interaction tests to need an allowlist entry rather than a rewrite. R4-R6 were the advisory W1-W3 before 2026-07-29.
