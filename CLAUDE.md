...
**Two invariants for concurrent peer-`make` safety on a shared objdir** (violating either one produces sporadic `undefined reference to 'main'` from `crt1.o:_start` — most easily reproduced under PyPy, where slower interpreter timing widens the window):
