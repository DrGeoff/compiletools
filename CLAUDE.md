...
Cache *keys* alone don't make CAS *artefacts* byte-identical across users — two users at different gitroot paths still embed their absolute checkout paths in DWARF debug info, `__FILE__` macros, and the `.d` dependency files the compiler emits, so the produced `.o` / `.exe` bytes diverge even when the cache key matches. `apptools._inject_ffile_prefix_map(args)` (called from the tail of `_commonsubstitutions`) appends `-ffile-prefix-map=<gitroot>=<target>` to `CXXFLAGS` and `CFLAGS` so the compiler rewrites those embedded paths to a workspace-location-independent prefix. `<target>` is the value of `--ffile-prefix-map-target` (default `.`, registered in `apptools.add_common_arguments`). `apptools._has_prefix_map_flag` does a per-slot substring scan for any of `-ffile-prefix-map=` / `-fdebug-prefix-map=` / `-fmacro-prefix-map=` / `-fcanon-prefix-map=` and skips injection on slots where the user already specified one — user choice wins. Resolved with `compiletools.git_utils.find_git_root()`; injection is a no-op when no gitroot is resolvable.

Linker symmetry: `apptools.canonicalize_for_command(tokens, anchor_root, *, target)` and `canonicalize_path_for_command` are sister functions of the cache-key canonicalizers — same parsing, but they substitute the configured target string instead of the `<GITROOT>` sentinel. `build_backend._create_link_rule` / `_create_shared_library_rule` route `merged_ldflags` / `ld_extra` / `extra_link_argv` through `canonicalize_for_command` so RPATHs, `--version-script` paths, and `-L` paths under the gitroot become target-prefixed in the emitted argv. The static-library rule has no LDFLAGS so it's untouched. `canonicalize_for_cache_key` also recognises the four prefix-map flag families (`-f{file,debug,macro,canon}-prefix-map=OLD=NEW`) and rewrites the LHS — without that, two users' auto-injected `-ffile-prefix-map=<their-gitroot>=.` tokens hash differently and pollute the cache key with the very prefix the flag is meant to scrub.

Acceptance gate: `test_ffile_prefix_map.py` builds the same sample twice in `home-alice/proj` and `home-bob/proj` (each with a `.git` marker so `find_git_root` resolves to the per-user workspace), then asserts byte-identity of the per-user CAS sub-tree. Parameterised over all six backends × four CAS layers; the helper compares the *content set* of file hashes, not the filename → hash mapping, because slurm sidecars per-invocation copies under hash-suffixed names whose filenames differ but whose bytes are identical. The `cas-objdir` and `cas-exedir` tests pass; `cas-pchdir` and `cas-pcmdir` are `xfail(strict=False)` because gcc embeds the absolute source path in PCH (`.gch`) and BMI (`.pcm`/`.gcm`) artefacts via its internal path-table, which is *not* subject to `-ffile-prefix-map`. Closing that gap requires either workspace-relative source paths in the precompile rule emitter (plus per-backend CWD discipline) or a `PWD=/proc/self/cwd` subprocess-env trick — `-fdebug-compilation-dir=` would address it for clang but is rejected by gcc as of 16.1.0. The `xfail` is non-strict so a future fix that closes the gap doesn't fail the suite until someone gets around to dropping the marker.
